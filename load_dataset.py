...
def load_dataset(dataset):
	if(dataset=="CIFAR-10"):
		...
		train_set.X = np.reshape(np.subtract(np.multiply(2./255.,train_set.X),1.),(-1,1,28,28))
		valid_set.X = np.reshape(np.subtract(np.multiply(2./255.,valid_set.X),1.),(-1,1,28,28))
		test_set.X = np.reshape(np.subtract(np.multiply(2./255.,test_set.X),1.),(-1,1,28,28))
		# flatten targets
		train_set.y = np.hstack(train_set.y)
		valid_set.y = np.hstack(valid_set.y)
		test_set.y = np.hstack(test_set.y)
